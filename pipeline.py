...
def align_bcs(i):
    ...
    if os.path.isfile(sam_file):
        ...
        subprocess.call(
            [
                f"{pwd}/scripts/barcode_align.sh",
                f"{outdir}/split/part_{i+1}_BCUMI.fasta.gz",
                f"{outdir}/{sample}_ref/",
                f"{outdir}/split/part_{i+1}_matching",
                cores,
                "-1",
            ]
        )
...
if mode == "5p10XGEX":
    ...
    if split:
        ...
        fqs = sorted(
            [
                f"{outdir}/split/" + f
                for f in os.listdir(f"{outdir}/split")
                if f.endswith("gz")
            ]
        )
        ...
        args = []
        for i in range(int(cores)):
            args.append((f"part_{i+1}", f"{outdir}/split"))
        ...
        subprocess.call(
            f"cat {outdir}/split/*_decon*.gz > {outdir}/{sample}_deconcat.fastq.gz",
            shell=True,
        )
        subprocess.call(
            f"cat {outdir}/split/*_BCUMI*.gz > {outdir}/{sample}_BCUMI.fasta.gz",
            shell=True,
        )
        ...
        subprocess.call(
            [f"{pwd}/scripts/align_trns.sh", cores, trns_ref, infile, outdir, sample]
        )
    ...
    subprocess.call(
        [
            f"{pwd}/scripts/barcode_ref.sh",
            f"{outdir}/{sample}_bcreads.fasta",
            f"{outdir}/{sample}_ref/",
        ]
    )
    ...
    subprocess.call(
        [
            f"{pwd}/scripts/barcode_align.sh",
            f"{outdir}/{sample}_BCUMI.fasta.gz",
            f"{outdir}/{sample}_ref/",
            f"{outdir}/{sample}_matching",
            cores,
            "-1",
        ]
    )
    ...
    subprocess.call(
        f"samtools index -@{cores} {outdir}/{sample}_genome_tagged.bam", shell=True
    )
...
if mode == "5p10XTCR":
    ...
    if split:
        ...
        if os.path.isfile(decon_file):
            ...
            subprocess.call(
                f"cat {outdir}/split/*_eds*.csv > {outdir}/{sample}_eds.csv", shell=True
            )
...
if mode == "3pXCR_slideseq":
    ...
    if split:
        ...
        if os.path.isfile(merged_VDJ):
            ...
            subprocess.call(
                f"cat {outdir}/split/*_VDJ.fastq.gz > {outdir}/{sample}_VDJ.fastq.gz",
                shell=True,
            )
            subprocess.call(
                f"cat {outdir}/split/*_BCUMI.fasta.gz > {outdir}/{sample}_BCUMI.fasta.gz",
                shell=True,
            )
            subprocess.call(
                f"cat {outdir}/split/*_polyA.fasta.gz > {outdir}/{sample}_polyA.fasta.gz",
                shell=True,
            )
...
if mode == "3p10XTCR_nuc":
    ...
    if split:
        ...
        subprocess.call(
            f"cat {outdir}/split/*_VDJ*.gz > {outdir}/{sample}_VDJ.fastq.gz", shell=True
        )
        subprocess.call(
            f"cat {outdir}/split/*_BCUMI*.gz > {outdir}/{sample}_BCUMI.fasta.gz",
            shell=True,
