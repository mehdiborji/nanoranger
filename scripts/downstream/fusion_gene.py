...
for read in samfile.fetch():
    alignments.append([read.reference_start, read.reference_end, read.reference_name,
                   read.qlen, read.rlen, read.get_tag('CB'), read.get_tag('UB')])
    
seqs = pd.DataFrame(alignments)

seqs.columns = ['ref_start', 'ref_end', 'ref_name', 'query_length', 'read_length', 'BC', 'UMI']
